...
- **Task Relate** - an optional per-task dropdown that links a task to an existing **Task-ID**
  (shown as `Task-XXX / task name`); a task cannot relate to itself. Cards display the link as
  a `Relates Task-XXX` tag, and search includes the relate value.
...
- **New `Task Relate` column** (optional) linking a task to an existing **Task-ID**. In the task
  form it is a dropdown built from the current task IDs, each shown as `Task-XXX / task name`;
  a task cannot be related to itself. The value is saved with add/edit, displayed on the task
  card as a `Relates Task-XXX` link tag, and included in search. Demo data ships with sample
  links. Add a `Task Relate` column to the live TaskList sheet to use it there - the backend
  maps by header, so no code change is needed.
